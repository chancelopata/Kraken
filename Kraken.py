...
Options:
  ...
  --hostFile PATH             perform the wordlist attack over a cluster of computers using openMPI. This file is the same as the hostfile that OpenMPI can use when using mpirun. 
...
Examples:
  ...
  # Launch a wordlist attack by distributing the work over the computers listed in the hostfile. This example has the wordlist, the list of hashes and Kraken.py stored in
  # a folder called 'cloud' that is shared between all of the computers in hostfile.
  mpirun --hostfile ~/hostfile python3 ~/cloud/Kraken/Kraken.py --md5 --hashFile ~/cloud/someHashes.txt --wordList ~/cloud/rockyou.txt --hostFile ~/hostfile
'''
